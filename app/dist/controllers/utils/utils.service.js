...
class UtilService {
    ...
    static async launch(profile, options = {}) {
        ...
        console.log(`🔍 Verificando disponibilidad de Chromium para el perfil ${profile.id}...`);
        ...
        try {
            ...
            console.log(`✅ Perfil ${profile.id} lanzado exitosamente con Chromium`);
            return { context, page, browser: context.browser() };
        }
        catch (error) {
            console.error(`❌ Error al lanzar Chromium para el perfil ${profile.id}:`, error.message);
